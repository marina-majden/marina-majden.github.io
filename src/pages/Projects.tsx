    ...
    <section id='projects' className='py-10 md:py-14'>
        <div className='container mx-auto px-6'>
            ...
            <div className='grid lg:grid-cols-2 gap-8 mt-12'>
                {t.projects.items.map((project: ProjectItem, idx: number) => (
                    <Reveal
                        key={idx}
                        delay={idx * 100}
                                ...
                                <div className='flex flex-wrap gap-2 mt-auto'>
                                    {project.stack.map(
                                        (tech: string, i: number) => (
                                            <span
                                                key={i}
                                                className='px-3 py-1 bg-slate-800 text-xs font-mono text-cyan-200 rounded-full border border-slate-700 hover:border-cyan-500 transition-colors'>
                                                {tech}
                                            </span>
                                        )
                                    )}
